...
ParticleContext_t P1;

// A fountain spraying up in the middle of the screen
void ComputeParticles1()
{
    //P.TimeStep(1/2.0);P.StartingAge();ResetSourceState()

    // Set up the state.
    P1.Velocity(PDCylinder(pVec(0.0f , 0.0f, 0.05f), pVec(0.0f , 0.0f, 0.08f), 0.02f, 0.02f));
    //P.Velocity(PDLine(pVec(0.0f, 0.0f, 0.0f), pVec(0.0f, 0.0f, 2.0f)));
    P1.Color(PDLine(pVec(0.0f, 1.0f, 0.0f), pVec(0.0f, 1.0f, 1.0f)), PDPoint(pVec(0.5f)));
     P1.Size(10.0);
	
	
    // Generate particles along a very small line in the nozzle.
    P1.Source(100, PDLine(pVec(0.0, 0, 0.0f), pVec(0.0, 0, 4.4f)));
	
	//P.TimeStep(1/2.0);
	P1.Source(100, PDLine(pVec(2.0, 0, 0.0f), pVec(2.0, 0, 4.4f)));
	P1.Source(100, PDLine(pVec(-2.0, 0, 0.0f), pVec(-2.0, 0, 4.4f)));
    ...
    P1.Velocity(PDSphere(pVec(0.0f , 0.0f, 0.0f), 0.01f, 0.01f));
    P1.Color(PDLine(pVec(0.0f, 0.0f, 0.0f), pVec(0.0f, 0.0f, 1.0f)), PDPoint(pVec(0.5f)));
    P1.Source(100, PDSphere(pVec(2.0f,6.0f, 1.0f), 0.005f, 0.005f));
    P1.Sink(true, PDSphere(pVec(2.0f,6.0f, 1.0f),2.0,1.5));

    P1.Velocity(PDSphere(pVec(0.0f , 0.0f, 0.0f), 0.01f, 0.01f));
    P1.Color(PDLine(pVec(1.0f, 0.0f, 0.0f), pVec(1.0f, 1.0f, 0.0f)), PDPoint(pVec(0.5f)));
    P1.Source(100, PDSphere(pVec(-2.0f,6.0f, 1.0f), 0.005f, 0.005f));
    P1.Sink(true, PDSphere(pVec(-2.0f,6.0f, 1.0f),2.0,1.5));

    P1.Velocity(PDSphere(pVec(0.0f , 0.0f, 0.0f), 0.01f, 0.01f));
    P1.Color(PDLine(pVec(1.0f, 0.0f, 0.0f), pVec(0.0f, 1.0f, 0.0f)), PDPoint(pVec(0.5f)));
    P1.Source(100, PDSphere(pVec(5.0f,10.0f, 2.0f), 0.005f, 0.005f));
    P1.Sink(true, PDSphere(pVec(5.0f,10.0f, 2.0f),2.0,1.5));

    P1.Velocity(PDSphere(pVec(0.0f , 0.0f, 0.0f), 0.01f, 0.01f));
    P1.Color(PDLine(pVec(1.0f, 0.0f, 0.0f), pVec(1.0f, 0.0f, 0.0f)), PDPoint(pVec(0.5f)));
    P1.Source(100, PDSphere(pVec(-5.0f,10.0f, 2.0f), 0.005f, 0.005f));
    P1.Sink(true, PDSphere(pVec(-5.0f,10.0f, 2.0f),2.0,1.5));
...
void DrawGroupAsPoints1()
{
    size_t cnt = P1.GetGroupCount();
    if(cnt < 1) return;
    ...
    cnt = P1.GetParticlePointer(ptr, flstride, pos3Ofs, posB3Ofs,
...
int main2(int argc, char **argv)
{
    ...
    int particle_handle = P1.GenParticleGroups(1, 10000);

    P1.CurrentGroup(particle_handle);
